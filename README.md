...
##### 原始模型逻辑：
1. 获取所需数据: 两个实体的llm特征文本数据(tokenized)、两个实体的距离数据、两个实体的邻居信息(未tokenized)
2. 转移到gpu
3. 扩展维度
4. 使用llm对两个实体进行特征提取
...
##### 改进点
1. 提取数据特征：
- 两个实体的llm特征文本数据(tokenized，mask)
- 各自拼接邻居后再使用llm提取的特征文本数据(tokenized，mask)
- 两个实体的距离数据
- 各自实体拼接邻居后经过多头注意力机制(Multi-Head Attention)提取特征
