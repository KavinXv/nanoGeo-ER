...
class GeoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 获取实体和邻居信息
        data = self.neigh_x[idx]
        e1, e2 = data["name1"], data["name2"]
        neigh1, neigh2 = data["neigh1"], data["neigh2"]

        # **拼接邻居信息**
        e1_text = e1 + ' ' + ' '.join(neigh1) if neigh1 else e1
        e2_text = e2 + ' ' + ' '.join(neigh2) if neigh2 else e2

        # **BERT 分词（邻居信息）**
        encoded = self.tokenizer(
            '[CLS] ' + e1_text + ' [SEP] ' + e2_text + ' [SEP]', 
            padding='max_length', truncation=True, max_length=self.max_seq_len, 
            return_tensors="pt"
        )

        return {
            'input_ids': torch.tensor(self.data_x[idx], dtype=torch.long),  # **原始的文本数据（BERT tokenized）**
            'n_encoded': encoded['input_ids'].squeeze(0),  # **新的邻居信息 tokenized 数据**
            'coords': torch.tensor(self.coord_x[idx], dtype=torch.float),  # 坐标数据
            'neighbors': data,  # 邻域数据（保留原始 JSON 格式）
...
def custom_collate_fn(batch):
    ...
    n_encoded = torch.stack([item['n_encoded'] for item in batch])
