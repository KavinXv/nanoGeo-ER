...
from model import GeoER, GeoConfig  # 导入GeoER模型和GeoConfig
...
model_args = dict(lm_hidden = lm_hidden, c_em = c_em, n_em = n_em, a_em = a_em)
config = GeoConfig(**model_args)
...
for epoch in range(start_epoch, epochs):
    ...
    for step, batch in enumerate(train_loader, 1):
        opt.zero_grad()
        x, x_encode, x_coord, x_n, y = batch['input_ids'].to(device), batch['n_encoded'].to(device), batch['coords'].to(device), batch['neighbors'], batch['labels'].to(device)
        att_mask = (x != 0).int().to(device)
        neigh_mask = (x_encode != 0).int().to(device)
        
        pred = model(x, x_encode, x_coord, x_n, att_mask, neigh_mask)
        ...
        # 打印当前步的损失
        print('Epoch:',epoch + 1,'Step:', step, '/', train_steps, 'Loss:', loss.item(), 'Time:', dt2)
        logger.info(f'Epoch: {epoch + 1} Step: {step}/{train_steps} | Loss: {loss.item()} | Time:{dt2}')
    t12 = time.time()
    dt1 = t12 - t1
    t1 = t12
    ...
    for batch in valid_loader:
        y, x, x_encode, x_coord, x_n = batch['labels'].view(-1).to(device), batch['input_ids'].to(device), batch['n_encoded'].to(device), batch['coords'].to(device), batch['neighbors']
        att_mask = (x != 0).int().to(device)
        neigh_mask = (x_encode != 0).int().to(device)
        
        pred = model(x, x_encode, x_coord, x_n, att_mask, neigh_mask, training=False)
        loss = criterion(pred, y)  # 计算损失
        pred_labels = torch.argmax(pred, dim=1)

        print('Val_Epoch:', (epoch+1)/val_epoch,'Step:', valid_step, '/', valid_steps, 'Loss:', loss.item(), 'Time:', dt3)
        logger.info(f'Val_Epoch:{(epoch+1)/val_epoch} Step: {valid_step}/{valid_steps} | Loss: {loss.item()} | Time:{dt3}')
